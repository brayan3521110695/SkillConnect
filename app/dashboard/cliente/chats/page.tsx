...
export default function ChatsClientePage() {
  ...
  useEffect(() => {
    const fetchConversaciones = async () => {
      try {
        const res = await fetch(`/api/conversaciones?clienteId=${usuario?._id}`);
    ...
    if (usuario?._id) fetchConversaciones();
  }, [usuario]);
  ...
  useEffect(() => {
    const fetchMensajes = async () => {
      ...
      try {
        const res = await fetch(`/api/mensajes?conversacionId=${selectedChat._id}`);
        const data = await res.json();
        setMensajes(data);
      } catch (error) {
    ...
    <div className="flex flex-col lg:flex-row h-screen bg-white relative">
      ...
      >
        ...
        {conversaciones.map((chat) => (
          ...
          >
            <img src={chat.user.jpg} className="w-10 h-10 rounded-full object-cover" />
            <div className="flex-1">
              <div className="flex justify-between items-center text-sm">
                <span className="font-semibold text-gray-800">{chat.nombre}</span>
              </div>
              <p className="text-gray-600 text-sm truncate">{chat.ultimoMensaje || "Inicia la conversación..."}</p>
            </div>
          </div>
        ))}
      </aside>
